(function() {
  ...
  _.uniq = function(array, isSorted, iterator) {
    ...
    _.each(array, function(arrayElement) {
      var duplicateCheck = _.indexOf(uniqArray, arrayElement, isSorted);
      if(duplicateCheck === -1) {
        uniqArray.push(arrayElement);
      }
    });
    if (iterator !== undefined) {
      var uniqIteratorArray = [];
      _.each(uniqArray, function(uniqElement) {
        if (iterator(uniqElement)) {
          trueCounter++;
          if(trueCounter === 1) {
            uniqIteratorArray.push(uniqElement);
          }
        } else {
          falseCounter++;
          if (falseCounter === 1) {
